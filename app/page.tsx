...
export default function Home() {
 ...
 useEffect(()=>{let dispose:(()=>void)|undefined,gone=false;import('./forest').then(({createForest})=>{if(gone||!host.current)return;const forest=createForest(host.current,()=>setReady(true));dispose=()=>forest.dispose();}).catch(()=>setError('Não foi possível iniciar a floresta. Recarregue em um navegador com WebGL.'));return()=>{gone=true;dispose?.();};},[]);
